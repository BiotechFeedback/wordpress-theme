<?php if ( ! is_page_template( 'template-campaign.php' ) ) { ?>
<footer class="content-info" role="contentinfo">
  <div class="container">
	<div class="row">
	  	<h3>biotech feedback </h3>
	  	<hr/>
	  	<div class="col-sm-3 col-xs-6">
	  		<h4>HOSTED BY</h4> 
            <a target="_blank" href="http://worldfoodcenter.ucdavis.edu/What_We_Do/IFAL.html"><img src="<?php echo get_stylesheet_directory_uri(); ?>/dist/images/IFALSig_FL2.png" class="sponser"></a>
	  		<!--<p class="small">The World Food Center Institute for Food and Agricultural Literacy (IFAL) at UC Davis</p> -->
	  	</div>
	  	<div class="col-sm-3 col-xs-6">
	  		<h4>SUPPORTED BY</h4>
	  		<p class="small">The UC Davis Chancellor’s Postdoctoral Fellowship Program</p>
	  		<!--<p class="small">Generous support from the public. <a href="contribute/">Donate today</a>!</p>-->
	  		<!--</ul>-->
	  	</div>
	  	<div class="col-sm-3 col-xs-6">
	  		<h4>IN PARTNERSHIP WITH</h4>
	  		<p ><a target="_blank" href="http://climatefeedback.org/" align="left"><img src="<?php echo get_stylesheet_directory_uri(); ?>/dist/images/cf_logo_c.png" class="sponser"></a>
	  		Climate Feedback </p>
	  	</div>
	  	<div class="col-sm-3 col-xs-6">
			<h4 >CONTACT US</h4>
			<p class="small">Biotech Feedback · One Shields Ave · 216 Robbins Hall · Davis, CA 95616</p>
			<p class="small"><a href="mailto:feedback@climatefeedback.org">feedback@biotechfeedback.org</a></p>
            <!-- / 
<h4 class="spaceup05">FOLLOW US</h4>
            <div class="social-footer">
                <a href="https://www.facebook.com/BiotechFeedback/" target="_blank"><i class="fa fa-facebook"></i></a>
                <a href="https://twitter.com/BiotechFdbk" target="_blank"><i class="fa fa-twitter"></i></a>
 -->
            </div>
	  	</div>
  	</div>
  </div>
</footer>
<?php } ?>
